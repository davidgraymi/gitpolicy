...
func (i *Indexer) checkOldIndexes(ctx context.Context) {
	for v := 0; v < i.version; v++ {
		...
		if err == nil && exists {
			log.Warn("Found older elasticsearch index named %q, Bindersnap will keep the old NOT DELETED. You can delete the old version after the upgrade succeed.", indexName)
		}
	}
}
